        ...
        if (isRun(cfg, run.getResult()))
        {
            ...
            do
            {
                ...
                try
                {
                    ...
                    Protocol.HTTP.send(expandedUrl, Format.JSON.serialize(jobState), target.getTimeout(), true);
                }
                catch (Throwable error)
                {
                    ...
                    {
                        listener.getLogger()
                                .println(String.format("Reattempting to notify endpoint with %s (%d tries remaining)",
